...
namespace RTGC {
  ...
  static ReferenceType __getRefType(oop ref_p) {
    Klass* k = ref_p->klass();
    assert(k->is_instance_klass(), PTR_DBG_SIG, PTR_DBG_INFO(ref_p));
  ...
  public:  
    ...
    void clear_weak_soft_garbage_referent() {
      ...
      rtgc_log(LOG_OPT(3),
          ...
          referent->getNodeInfo().hasMultiRef());
...
static void __keep_alive_final_referents(OopClosure* keep_alive, VoidClosure* complete_gc) {
  ...
  for (RefIterator<is_full_gc> iter(g_finalList); (ref = iter.next_ref(SkipNone)) != NULL; ) {
    ...
    if (!is_alive) {
      ...
      postcond(!referent->isGarbageMarked());
      postcond(!referent->getNodeInfo().hasSafeAnchor() || (!CLEAR_FINALIZE_REF && referent->getNodeInfo().getSafeAnchor() != ref));
      postcond(!referent->getNodeInfo().hasShortcut());
