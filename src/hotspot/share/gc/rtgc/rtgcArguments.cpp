...
size_t RtgcArguments::conservative_max_heap_alignment() {
  return UseLargePages ? os::large_page_size() : os::vm_page_size();
}

void RtgcArguments::initialize() {
  ...
  if (RtgcMaxTLABSize < MinTLABSize) {
    log_warning(gc)("RtgcMaxTLABSize < MinTLABSize, adjusting it to " SIZE_FORMAT, MinTLABSize);
    RtgcMaxTLABSize = MinTLABSize;
  }

  if (!RtgcElasticTLAB && RtgcElasticTLABDecay) {
    log_warning(gc)("Disabling RtgcElasticTLABDecay because RtgcElasticTLAB is disabled");
    FLAG_SET_DEFAULT(RtgcElasticTLABDecay, false);
  }
...
void RtgcArguments::initialize_alignments() {
...
CollectedHeap* RtgcArguments::create_heap() {
  return new RtgcHeap();
}
