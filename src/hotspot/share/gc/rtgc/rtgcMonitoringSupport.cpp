...
class RtgcSpaceCounters: public CHeapObj<mtGC> {
...
public:
  RtgcSpaceCounters(const char* name,
                 int ordinal,
                 size_t max_size,
...
class RtgcGenerationCounters : public GenerationCounters {
private:
  RtgcHeap* _heap;
public:
  RtgcGenerationCounters(RtgcHeap* heap) :
...
RtgcMonitoringSupport::RtgcMonitoringSupport(RtgcHeap* heap) {
  _heap_counters  = new RtgcGenerationCounters(heap);
  _space_counters = new RtgcSpaceCounters("Heap", 0, heap->max_capacity(), 0, _heap_counters);
}

void RtgcMonitoringSupport::update_counters() {
  MemoryService::track_memory_usage();

  if (UsePerfData) {
    RtgcHeap* heap = RtgcHeap::heap();
